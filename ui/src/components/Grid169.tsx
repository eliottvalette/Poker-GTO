...
import { ACTIONS, ACTIONS_L, ACTION_COLORS, GridMix } from "@/src/lib/policy";

const CARD_LABS = ["A","K","Q","J","T","9","8","7","6","5","4","3","2"];

function Cell({ mix, labelThresholdPct=18 }: { mix: GridMix; labelThresholdPct?: number }) {
  const order = ACTIONS.map((a,idx)=>({ upper:a, lower:ACTIONS_L[idx], p: mix?.[a] ?? 0 }));
  let cursor = 0;
  return (
    <div className="relative w-16 h-10 bg-zinc-900 border border-white/10 hover:outline hover:outline-1 hover:outline-white/40">
      {order.map(({upper, lower, p})=>{
        if (p <= 0) return null;
        const left = `${cursor*100}%`; const width = `${p*100}%`;
        cursor += p;
        return (
          <div key={upper}
